...
import { IQuestion } from '@word-coach/react'
import { querySerpdog } from './utils/serpdog-images'
...
export class AIQuestions {
  ...
  static async StreamResponse(
    ...
    const processQuestion = async (question: IQuestion): Promise<IQuestion> => {
    ...
    try {
      while (true) {
        ...
        if (done) {
          ...
          const processedQuestion = await processQuestion(question)
   ...
   */
  static async replaceImageDescriptionWithUrl(
    question: IQuestion
  ): Promise<any> {
    if (question.type === QuestionTypes.TEXT_WITH_IMAGE) {
      const result = await querySerpdog(question.image)
      return { ...question, image: result.image_results[1].original }
    }
      ...
      return {
        ...
        options: results.map((result) => {
          if (result.status === 'fulfilled') {
            return result.value.image_results[1].original
          }
