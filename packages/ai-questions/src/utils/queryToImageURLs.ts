...
export const queryToImageURLs = async (query: string) => {
   ...
   * ie = encoding
   */
  const queryString = `https://www.google.com/search?q=${query}&oq=${query}&hl=en&tbm=isch&asearch=ichunk&async=_id:rg_s,_pms:s,_fmt:pc&sourceid=chrome&ie=UTF-8&tbs=iar:w,ift:jpg`
